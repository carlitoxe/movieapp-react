        ...
        {loading ? (
          ...
          <ul className="text-center mt-2">
            {categories?.map((category) => {
              return (
                <li onClick={() => navigate(`category/${category.id}/${category.name}`)} key={category.id} className="cursor-pointer mt-2.5 mr-1.5 inline-flex text-white font-medium border border-solid border-white rounded-2xl py-1 px-3 hover:bg-sky-600">
                  {category.name}
                </li>
              );
            })}
